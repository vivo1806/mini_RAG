...
**Why this model?**
- **Fully local & private** — Ollama runs the LLM on your machine, so no document data is sent to external APIs during answer generation.
- **No API costs for generation** — Embedding calls use the OpenRouter API, but all LLM inference is free and local.
- **Instruction-following** — Llama 3 excels at following system prompts, which is critical for enforcing grounding constraints (see below).
- **Configurable** — You can swap in any Ollama-supported model (e.g., `mistral`, `llama3.2`) by changing `OLLAMA_MODEL` in `.env`.
